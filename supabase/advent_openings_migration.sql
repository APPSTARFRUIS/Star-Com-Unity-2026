-- Calendrier de l'Avent : ouverture unique par utilisateur et par case
...
create table if not exists public.advent_openings (
  ...
  constraint advent_openings_unique_user_day unique (animation_id, user_id, day_number)
);

create index if not exists idx_advent_openings_user on public.advent_openings(user_id, opened_at desc);
create index if not exists idx_advent_openings_animation on public.advent_openings(animation_id, day_number);
...
create policy "advent openings read" on public.advent_openings for select using (true);

drop policy if exists "advent openings insert" on public.advent_openings;
create policy "advent openings insert" on public.advent_openings for insert with check (true);

-- Fonction atomique : vérifie l'unicité, calcule le résultat et attribue les points dans une seule transaction.
...
begin
  ...
  returning id, advent_openings.opened_at into v_opening_id, v_opened_at;
  ...
  if v_type in ('quiz', 'mystery') then
    ...
    v_outcome := jsonb_build_object('isCorrect', v_is_correct, 'answer', coalesce(p_answer, ''));
  elsif v_type = 'instant' then
    v_probability := least(greatest(coalesce((v_day->>'winProbability')::numeric, 0), 0), 100);
  ...
  update public.engagement_animations
  set participants = case
    when p_user_id = any(coalesce(participants, '{}'::uuid[])) then participants
    else array_append(coalesce(participants, '{}'::uuid[]), p_user_id)
  end
...
grant execute on function public.open_advent_day(uuid, uuid, integer, text) to anon, authenticated;
